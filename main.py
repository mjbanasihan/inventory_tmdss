    ...
    try:
        if search:
            rows = db.execute(text(
                "SELECT * FROM inventory_items WHERE LOWER(supply_name) LIKE LOWER(:s) OR LOWER(COALESCE(variety,'')) LIKE LOWER(:s) OR LOWER(COALESCE(date_received,'')) LIKE LOWER(:s) OR LOWER(COALESCE(po_number,'')) LIKE LOWER(:s) ORDER BY date_received DESC NULLS LAST, id DESC"
...
def get_given_out(search: str = "", db: Session = Depends(get_db)):
    if search:
        rows = db.execute(text(
            "SELECT * FROM given_out_items WHERE supply_name ILIKE :s OR COALESCE(variety,'') ILIKE :s OR COALESCE(date_given,'') ILIKE :s OR COALESCE(po_number,'') ILIKE :s ORDER BY id"
        ), {"s": f"%{search}%"}).mappings().all()
    else:
        rows = db.execute(text(
