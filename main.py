...
def refresh_flags():
    ...
    print(f"Columns — inv.variety:{HAS_VARIETY} giv.variety:{HAS_VARIETY_GIVEN} inv.po:{HAS_PO_INV} giv.po:{HAS_PO_GIVEN} txn.po:{HAS_PO_TXN}", flush=True)
    ...
    try:
        if search:
            rows = db.execute(text(
                "SELECT * FROM inventory_items WHERE LOWER(supply_name) LIKE LOWER(:s) OR LOWER(COALESCE(po_number,'')) LIKE LOWER(:s) ORDER BY date_received DESC NULLS LAST, id DESC"
...
def update_inventory_item(item_id: int, item: schemas.InventoryItemCreate, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if HAS_VARIETY and HAS_PO_INV:
            ...
            print(f"PUT inventory — variety={item.variety!r} po={item.po_number!r} saved", flush=True)
        elif HAS_VARIETY:
...
def get_given_out(search: str = "", db: Session = Depends(get_db)):
    if search:
        rows = db.execute(text(
            "SELECT * FROM given_out_items WHERE supply_name ILIKE :s OR COALESCE(po_number,'') ILIKE :s ORDER BY id"
        ), {"s": f"%{search}%"}).mappings().all()
    else:
        rows = db.execute(text(
